...
public class WebSteps {
    ...
    public static void IAmOnThePage(String pageName) {
        ...
        if(AllProducts.GOSFramePages.contains(pageName))
        	{ WebHelp.navigateTo(AllURLs.getModuleURL(pageName)); IWaitSomeSecs("20","pageName");
        	AssertExecutedStep(WebHelp.navigateTo(AllURLs.getModuleURL(pageName))  + " : " + "I navigate to the " + pageName + " page");
        	IWaitSomeSecs("20","pageName"); ISwitchToTheContent(pageName);}
    ...
    public static void ITakeScreenShot(String fileName) {
        AssertExecutedStep( WebHelp.takeScreenShot(System.getProperty("reportPath") + "/" + fileName) + " : " + "I take screenshot and save to " + System.getProperty("reportPath") + "/" + fileName);
    }
